...
class TagApiTestCase(LiveServerTestCase):
    def setUp(self):
        get_user_model().objects.create_user(email="testuser@example.com", password="password")
    ...
    def test_tag_create_basic_auth(self):
        ...
        resp = self.client.post(self.live_server_url + "/api/v1/tags/", {"value":"tag5"})
        self.assertEqual(resp.status_code, 201)
        self.assertEqual(Tag.objects.all().count(), 5)
    
    def test_tag_create_token_auth(self):
        token_resp = self.client.post(self.live_server_url + "/api/v1/token-auth/",
